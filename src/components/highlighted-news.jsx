...
export const HighlightedNews = ({ data }) => {
    if (!data) {
        return null
    }
    return (
        <>
            <section className="margin -mt-20">
                <Link
                    href={`/news/${slugify(data.category)}/${slugify(data.title)}`}
                    className="grid grid-cols-1 md:grid-cols-2 gap-10 group"
                >
