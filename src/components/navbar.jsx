            ...
            {isHome === true ? (
                <>
                    <li className='relative flex flex-col items-center group'>
                        <Link
                            href={href}
                            className={`${path === "/" ? 'opacity-100' : 'opacity-60 dark:opacity-80 hover:opacity-100'} relative text-[13px] rounded-full group-hover:not-[&:hover]:opacity-30 hover:scale-102 hover:font-medium hover:text-orange-400 duration-300`}
                            aria-current="page"
                        >
                            {children}
                        </Link>
                ...
                <>
                    <li className='relative flex flex-col items-center group'>
                        <Link
                            href={href}
                            className={`${path.startsWith(href) ? 'opacity-100' : 'opacity-60 dark:opacity-80 hover:opacity-100'} relative text-[13px] rounded-full group-hover:not-[&:hover]:opacity-30 hover:scale-102 hover:font-medium hover:text-orange-400 duration-300`}
                            aria-current="page"
                        >
                            {children}
                        </Link>
...
export const Navbar = ({ children }) => {
    ...
    return (
        <>
            ...
            <nav className={`${isScrolled ? "lg:left-12 lg:right-12 left-4 right-4" : "left-1.5 right-1.5 lg:left-9 lg:right-9"} hidden duration-300 fixed top-2  md:flex items-center justify-center z-555`}>
                ...
                <div
                    ...
                    <button
                        className={`${isScrolled ? "scale-100 translate-y-0" : "scale-0 -translate-y-full"} duration-300 origin-top hover:text-orange-400 cursor-pointer`}
                    >
